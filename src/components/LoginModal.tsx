...
const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, mode = 'login', onModeChange, selectedPlan = 'auto', billingCycle = 'monthly', initialEmail }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isForgotPassword) {
        ...
        const baseUrl = origin.includes('quittancesimple.fr') ? 'https://www.quittancesimple.fr' : origin;
        const { error: resetError } = await supabase.auth.resetPasswordForEmail(formData.email, {
          redirectTo: `${baseUrl}/set-password`
        });
        if (resetError) {
          setError(resetError.message || 'Une erreur est survenue. Vérifiez votre adresse email.');
          setIsLoading(false);
          return;
        }
      ...
      if (mode === 'signup') {
        ...
        try {
          await supabase.functions.invoke('send-welcome-email', {
            body: { email: formData.email, nom: '', prenom: '' },
          });
        } catch (e) {
        ...
        <div className="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full relative z-10">
          ...
          <div className="p-8">
            ...
            <h2 className="text-2xl font-bold text-[#415052] text-center mb-2">
              {isForgotPassword ? 'Réinitialiser votre mot de passe' : mode === 'signup' ? 'Créer un compte' : 'Connexion'}
            </h2>
            ...
            {forgotSuccess && (
              <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                <p className="text-sm text-green-800">
                  Un email avec un lien de réinitialisation a été envoyé à <strong>{formData.email}</strong>. Consultez votre boîte de réception (et les spams).
                </p>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                {mode === 'login' && (
                  <button
                    ...
                    className="text-xs text-[#2D3436] font-medium hover:underline mt-1"
                  >
                    Mot de passe oublié ?
                  </button>
                )}
              ...
              >
                {isLoading
                  ? (isForgotPassword ? 'Envoi...' : mode === 'signup' ? 'Création...' : 'Connexion...')
                  : isForgotPassword
              ...
              <p className="text-sm text-[#415052]">
                {isForgotPassword ? (
                  <>
                    <button
                      type="button"
                      onClick={() => { setError(''); setIsForgotPassword(false); setForgotSuccess(false); }}
