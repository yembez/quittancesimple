...
const QuittancePreview: React.FC<QuittancePreviewProps> = ({ formData, isElectronicSignatureChecked = false, onClose }) => {
  // Calcul automatique du total
  const loyer = parseFloat(formData.loyer) || 0;
  ...
  const getPeriodDates = (periode: string) => {
    ...
    if (formData.isProrata && formData.dateDebut && formData.dateFin) {
  ...
  const periodDates = getPeriodDates(formData.periode);
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="bg-blue-50 p-4 rounded-lg">
            <h3 className="font-bold text-blue-900 mb-2">BAILLEUR</h3>
            <div className="text-sm space-y-1">
              <p className="font-semibold">{formData.baillorName || '[Nom du bailleur]'}</p>
              <p>{formData.baillorAddress || '[Adresse du bailleur]'}</p>
              <p>{formData.baillorEmail || '[Email du bailleur]'}</p>
            </div>
          </div>
          
          <div className="bg-green-50 p-4 rounded-lg">
            <h3 className="font-bold text-green-900 mb-2">LOCATAIRE</h3>
            <div className="text-sm space-y-1">
              <p className="font-semibold">{formData.locataireName || '[Nom du locataire]'}</p>
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <p className="text-sm">{formData.logementAddress || '[Adresse du logement loué]'}</p>
        </div>

        {/* Déclaration */}
        <div className="bg-white border-2 border-gray-300 p-6 rounded-lg">
          <p className="text-sm leading-relaxed text-gray-800">
            Je soussigné(e) <strong>{formData.baillorName || '[Nom du bailleur]'}</strong>, 
            propriétaire du logement désigné ci-dessus, déclare avoir reçu de{' '}
            <strong>{formData.locataireName || '[Nom du locataire]'}</strong>, la somme de{' '}
            <strong className="text-lg text-blue-600">
              {total > 0 ? `${total.toFixed(2)} €` : '[Montant]'}
            </strong>{' '}
            ({total > 0 ? numberToFrench(total) : '[Montant en lettres]'}) 
        ...
        <div className="border border-gray-300 rounded-lg overflow-hidden">
          ...
          <div className="divide-y divide-gray-200">
            <div className="flex justify-between px-4 py-2">
              <span>Loyer</span>
              <span className="font-semibold">{loyer > 0 ? `${loyer.toFixed(2)} €` : '0,00 €'}</span>
            </div>
            <div className="flex justify-between px-4 py-2">
              <span>Provision pour charges</span>
              <span className="font-semibold">{charges > 0 ? `${charges.toFixed(2)} €` : '0,00 €'}</span>
            </div>
            <div className="flex justify-between px-4 py-3 bg-gray-50 font-bold text-lg">
              <span>TOTAL</span>
              <span className="text-blue-600">{total > 0 ? `${total.toFixed(2)} €` : '0,00 €'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end">
          <div className="text-center space-y-4">
            ...
            <div className="text-sm text-gray-600 space-y-1">
              <p><strong>Fait à :</strong> {(() => {
                const address = formData.baillorAddress || '';
            ...
            {isElectronicSignatureChecked ? (
              <div className="relative">
                ...
                <p className="text-xs text-gray-600 mt-2">{formData.baillorName}</p>
              </div>
            ) : (
