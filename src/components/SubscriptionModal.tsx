...
const SubscriptionModal: React.FC<SubscriptionModalProps> = ({ isOpen, onClose, plan }) => {
  ...
  const monthlyTotal = numberOfTenants === 1
    ? currentPlan.basePrice
    : currentPlan.basePrice + (numberOfTenants - 1) * currentPlan.additionalPrice;
    ...
    try {
      ...
      const { error: insertError } = await supabase
        ...
        .upsert({
          ...
          plan_actuel: plan === 'auto' ? 'Quittance Automatique' : 'Quittance Automatique+',
  ...
  const handleGoToDashboard = () => {
    ...
    // Plan Automatique+ (bank) -> Configuration complète avec locataires puis synchro bancaire
