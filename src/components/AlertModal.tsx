  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 animate-fade-in">
        <div className={`p-6 rounded-t-2xl ${type === 'error' ? 'bg-red-50' : 'bg-green-50'}`}>
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              ...
              <h3 className={`text-lg font-bold ${type === 'error' ? 'text-red-900' : 'text-green-900'}`}>
                {title}
              </h3>
            </div>
            <button
          ...
          <ul className="space-y-2">
            {messages.map((message, index) => (
              <li key={index} className="flex items-start gap-2 text-gray-700">
                <span className={`mt-1 w-1.5 h-1.5 rounded-full flex-shrink-0 ${type === 'error' ? 'bg-red-500' : 'bg-green-500'}`} />
