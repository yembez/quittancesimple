...
const [recentTransactions, setRecentTransactions] = useState<any[]>([]);
  ...
  const handleStripeReturn = async () => {
    ...
    if (success === 'true' && sessionId) {
      try {
        // Récupérer l'email depuis l'URL, localStorage ou la session
        let email = emailParam || localStorage.getItem('proprietaireEmail');
        ...
        if (!email) {
          console.error('Email non trouvé après paiement');
          alert('Erreur : impossible de finaliser le paiement. Veuillez vous reconnecter.');
          navigate('/');
          return;
        }

        // Récupérer le proprietaire par email
        ...
        if (!proprietaireData) {
          ...
          alert('Erreur : compte introuvable. Veuillez vous reconnecter.');
          navigate('/');
          return;
        }

        // Compter les locataires avec le bon proprietaire_id
        ...
        const { error: updateError } = await supabase
          ...
          .update({
            ...
            nombre_locataires: locatairesCount || 0
          })
          .eq('id', proprietaireData.id);
      ...
      const email = emailParam || localStorage.getItem('proprietaireEmail');
...
setRecentTransactions(txData || []);
  ...
  const handleBillingCycleConfirm = async (billingCycle: 'monthly' | 'yearly') => {
    ...
    // Set missing fields and show warning modal
  ...
  const handleConfirmPayment = async () => {
    ...
    const billingCycle = pendingBillingCycle;
    ...
    try {
      ...
      const { data: session, error: checkoutError } = await supabase.functions.invoke('stripe-checkout', {
        body: {
          ...
          mode: 'subscription',
          success_url: `${window.location.origin}/billing?session_id={CHECKOUT_SESSION_ID}&success=true&nb_locataires=${locataires.length}&email=${encodeURIComponent(proprietaire.email)}`,
          cancel_url: `${window.location.origin}/dashboard?canceled=true&email=${encodeURIComponent(proprietaire.email)}`
        }
      });

      if (checkoutError) {
        console.error('Checkout error details:', checkoutError);
        alert('Erreur lors de la redirection vers le paiement: ' + (checkoutError.message || JSON.stringify(checkoutError)));
        return;
      }
  ...
  const getPlanName = () => {
    ...
    if (count <= 2) return 'Quittance Automatique (1-2 locataires)';
    if (count <= 5) return 'Quittance Automatique (3-5 locataires)';
    return 'Quittance Automatique (5+ locataires)';
  };
