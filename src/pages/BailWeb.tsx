...
interface BailData {
  ...
  duree_contrat: '3ans' | '6ans' | 'reduite' | '';
...
const STORAGE_KEY = 'bail_web_draft';
const STORAGE_KEY_SAVED_LIST = 'bail_vide_saved_list';

export interface SavedBailItem {
  id: string;
  label: string;
  data: BailData;
  createdAt: number;
...
export default function BailWeb() {
  ...
  // Charger la liste des baux sauvegardés
  useEffect(() => {
    try {
  ...
  const getBailDisplayName = (bailData: BailData) => {
    ...
    return `Bail vide - ${namesStr} - ${day}-${month}-${year} ${hour}-${min}`;
  };
  ...
  const loadSignedBailAsModel = (bailId: string) => {
    try {
      const raw = localStorage.getItem(`bail_vide_data_${bailId}`);
      if (!raw) return;
  ...
  const handleExportPDF = async (options: { download?: boolean; uploadForSignature?: boolean; uploadToDocuments?: boolean } = {}) => {
    ...
    try {
      ...
      if (download) {
        pdf.save('Bail_Location_Vide_ALUR_QuittanceSimple.pdf');
      }
  ...
  const handleFinalizeForSignature = async () => {
    ...
    localStorage.setItem(`bail_vide_data_${bailId}`, JSON.stringify(data));
  ...
  return (
    <>
      ...
      {/* Header */}
        ...
        {/* Contract Content - Multiple A4 Pages (WebMirror) - scroll interne pour éviter débordement */}
...
<p className="font-medium text-gray-700">{getBailFamilyNamesLabel(req)}</p>
                        ...
                        {signatureRequests.filter((r) => r.status === 'signed').map((req) => (
                          <li key={req.id} className="flex items-center justify-between gap-4 bg-white border border-green-200 rounded-lg px-4 py-3 shadow-sm">
                            ...
                            <div className="flex items-center gap-2 flex-shrink-0">
                              {typeof localStorage !== 'undefined' && localStorage.getItem(`bail_vide_data_${req.bail_id}`) && (
                                <button
                                  type="button"
          ...
          <div ref={contractRef} className="flex flex-col items-center min-h-0 pb-8">
            ...
            <A4Page pageNumber={1}>
              ...
              <div className="text-center mb-4">
                ...
                <h2 className="bail-section-title border-b-0 pb-0 text-center text-base mt-2">LOCAUX VIDES À USAGE D'HABITATION</h2>
              </div>
          ...
          <section className="mb-8">
            ...
            <div className="mb-4">
              ...
              {data.chauffage_type === 'collectif' && (
                <div className="ml-4">
                  <span className="text-base text-gray-700">(Si collectif, préciser les modalités de répartition) : </span>
                  <InlineInput
          ...
          <section className="mb-8">
            ...
            <p className="bail-legal-intro text-gray-600 mb-4">
        ...
        {/* Sticky bottom bar - Créer un bail */}
        {activeTab === 'creer' && (
