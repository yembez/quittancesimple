 ...
 * - SMSMode (default, cost-effective)
 * - Octopush (backup)
 * - Twilio (backup, widely used)
 * - Brevo (legacy, expensive)
 *
 * Configuration via Supabase Vault secrets:
 * - sms_provider: 'smsmode' | 'octopush' | 'twilio' | 'brevo' (default: 'smsmode')
 *
 * SMSMode:
 * - smsmode_api_key
 *
...
export async function sendSMS(message: SMSMessage): Promise<SMSResult> {
  ...
  switch (provider) {
    ...
    default:
      result = {
        success: false,
        error: `Unknown SMS provider: ${provider}. Valid options: smsmode, octopush, twilio, brevo`,
      };
  }
