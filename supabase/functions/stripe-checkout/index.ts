...
Deno.serve(async (req) => {
  try {
    ...
    // Validate that either price_id OR line_items is provided (not both)
    ...
    if (line_items) {
      ...
      for (const item of line_items) {
        if (!item.price || typeof item.price !== 'string') {
          return corsResponse({ error: 'Each line item must have a valid price string' }, 400);
        }
        if (!item.quantity || typeof item.quantity !== 'number' || item.quantity < 1) {
          return corsResponse({ error: 'Each line item must have a valid quantity >= 1' }, 400);
        }
      }
    }
